...
if ($file_status_meeting_notice && $file_status_meeting_procedure_manual && $file_status_major_shareholders && $file_status_annual_report && $file_status_minutes) {
    ...
    $sql = " update shareholders_meeting_en set years=$years, name='$name', date='$date', location='$location', link_video='$link_video', status=$status ";
    ...
    if(!empty($fileName_meeting_procedure_manual)){
        $meeting_procedure_manual = "http://".$_SERVER['SERVER_NAME']."/admin/dist/images/shareholders_meeting_en/$name/".$fileName_meeting_procedure_manual;
    ...
    $sql = " insert into edit_log set user='$admin', menu='編輯股東會-英文', datetime='$now' ";
    $pdo->query($sql);
    echo "<script>alert('資料已更新');window.location.href='shareholders_meeting_en.php';</script>";
}else{
